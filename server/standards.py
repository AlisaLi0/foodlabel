...
def extract_system() -> str:
    ...
    return "\n".join(
        [
            "你是食品标签识读助手。用户会给出一件预包装食品标签由**一个或多个 OCR 引擎**识别出的"
            "**文本**（可能有错漏、乱序、粘连，或多个结果互相不一致），有时还会附上**原始标签图片**。",
            "请综合这些材料，整理、归并出标签上的全部信息，提取为结构化字段：",
            "- 若提供了原始图片，**以图片为准**，OCR 文本作辅助；多个 OCR 结果不一致时，取更符合图片、更合理的版本。",
            "- 若只有文本，则据文本互相印证、纠错后整理。",
            "只识读、整理文字，**不做合规判断、不推理、不评价**。材料中没有的字段留空，**不要臆造**。",
            "- barcode 填条形码下方的数字串（通常为 13 位 EAN-13 数字），识别不到就留空，切勿臆造。",
            "所有文字用简体中文。只输出一个 JSON 对象：",
            "{",
            '  "is_food_label": true/false,',
            '  "label_type": "如：预包装食品标签/进口食品/营养标签豁免类 等",',
            '  "extracted": {',
            '    "food_name":"", "ingredients":"", "additives":"", "net_content":"", "barcode":"",',
